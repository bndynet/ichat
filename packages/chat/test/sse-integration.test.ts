...
function textOf(
  chat: Chat<Record<`x-${string}`, unknown>>,
  msgId: string,
  partId: string,
): string {
...
// explicit patch object
{
  ...
  assert.equal(
    chat.messages[0].parts!.find((p) => p.id === "p1")?.status,
    "complete",
  );
}
...
// tool-call state update
{
  ...
  assert.ok(r.ok);
  assert.equal(
    (chat.messages[0].parts!.find((p) => p.id === "tc1") as { state?: string })
      .state,
    "output-available",
  );
}
...
// JSON string payload
{
  ...
  chat.addMessage({
    ...
    role: "assistant",
    parts: [
      todoPart([{ id: "it1", title: "T", status: "pending" }], {
        id: "t1",
        revision: 1,
      }),
    ],
...
// full text streaming lifecycle
{
  ...
  assert.equal(
    chat.messages[0].parts!.find((p) => p.id === "body")?.status,
    "complete",
  );
}
