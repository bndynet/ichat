...
declare global {
  interface Window {
    __ICHAT_VIRTUAL_BENCHMARK__?: { status: 'running'; progress?: string } | VirtualBenchmarkReport;
  }
}
...
async function settleVirtualList(element: ChatMessages): Promise<LitVirtualizer | undefined> {
  ...
  const laidOut = await waitForCondition(
    () =>
      virtualizer.clientHeight > 0 && Boolean(element.shadowRoot?.querySelector('i-chat-message')),
  );
...
async function validateFunctions(): Promise<VirtualFunctionalValidation> {
  ...
  const regularToVirtualAnchorPreserved =
    Boolean(anchorMessageId) &&
    (await waitForCondition(() => firstVisibleMessageId(virtual, scroller) === anchorMessageId));
  ...
  scroller?.dispatchEvent(new Event('scroll'));
  const autoScrollBeforeAppend = (virtual as unknown as { _autoScroll: boolean })._autoScroll;
  virtual.messages = [
    ...virtual.messages,
    {
  ...
  const autoScrollBeforeUpdate = (virtual as unknown as { _autoScroll: boolean })._autoScroll;
  virtual.messages = [
