...
import {
  ...
  injectPluginCss,
} from "../src/renderers/plugin-styles.js";
...
test("styles are collected into combined CSS", () => {
  ...
  assert.ok(
    ...
    "CSS should accumulate, not replace",
  );
});
...
test("globalStyles are collected into combined global CSS", () => {
  ...
  assert.ok(
    globalCss.includes("@font-face"),
    "global CSS should contain @font-face",
  );
});

test("styles and globalStyles are isolated from each other", () => {
  registerMarkdownPlugin(
    makePlugin("test-isolation", ".shadow { }", "@font-face { }"),
  );
  const styles = getMarkdownPluginStyles();
  const global = getMarkdownPluginGlobalStyles();
  assert.ok(
    !styles.includes("@font-face"),
    "shadow styles should not contain @font-face",
  );
  assert.ok(
    !global.includes(".shadow"),
    "global styles should not contain .shadow",
  );
});
...
test("part renderer duplicate name warns and keeps the first registration", () => {
  ...
  try {
    const warnings = captureWarnings(() =>
      partRendererRegistry.register(second),
    );
    assert.equal(warnings.length, 1);
    assert.equal(
