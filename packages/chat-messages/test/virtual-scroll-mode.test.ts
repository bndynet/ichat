...
import {
  ...
  ChatMessages,
} from "../src/components/chat-messages.js";
...
test("omitted virtualScroll falls back to the 'auto' default", () => {
  ...
  assert.equal(
    makeElement({}, AUTO_VIRTUAL_THRESHOLD + 1)._virtualScrollEnabled(),
    true,
  );
  assert.equal(
    makeElement({}, AUTO_VIRTUAL_THRESHOLD)._virtualScrollEnabled(),
    false,
  );
});
...
test("'auto' switches at the message-count threshold", () => {
  ...
  assert.equal(
    makeElement(config, AUTO_VIRTUAL_THRESHOLD)._virtualScrollEnabled(),
    false,
  );
  assert.equal(
    makeElement(config, AUTO_VIRTUAL_THRESHOLD + 1)._virtualScrollEnabled(),
    true,
  );
});
...
test("explicit booleans ignore the threshold", () => {
  assert.equal(
    makeElement({ virtualScroll: true }, 1)._virtualScrollEnabled(),
    true,
  );
  assert.equal(
    makeElement(
      { virtualScroll: false },
      AUTO_VIRTUAL_THRESHOLD + 1,
