...
test("inline SVG avatars use the sanitized result and fail closed", () => {
  ...
  assert.deepEqual(receivedConfig?.USE_PROFILES, {
    svg: true,
    svgFilters: true,
  });
...
test("link protocols use safe defaults and require custom schemes to opt in", () => {
  for (const href of [
    ...
    assert.equal(
      ...
      true,
      `${href} should be allowed by default`,
    );
    assert.equal(
      isAllowedLinkHref(href, []),
      true,
      `${href} should be allowed with an empty list`,
    );
  }
  ...
  for (const href of [
    ...
    assert.equal(
      isAllowedLinkHref(href),
      false,
      `${href} should be blocked by default`,
    );
    assert.equal(
      isAllowedLinkHref(href, []),
      false,
  ...
  assert.equal(
    isAllowedLinkHref("myapp://example", ["https:", "myapp:"]),
    true,
  );
  assert.equal(
    isAllowedLinkHref("https://example.test", ["https", "myapp"]),
    true,
  );
  assert.equal(
    isAllowedLinkHref("http://example.test", ["https", "myapp"]),
    false,
  );
  assert.equal(
    isAllowedLinkHref("javascript:alert(1)", ["https", "myapp"]),
    false,
  );
...
test("part factories preserve explicit stable ids and getMessageText stays text-only", () => {
  ...
  const todo = todoPart(
    [{ id: "task-1", title: "Ship it", status: "pending" }],
    {
      id: "todo-1",
      title: "Plan",
      revision: 3,
  ...
  assert.equal(
    getMessageText({ id: "m-1", role: "assistant", parts }),
    "Hello\n\nWorld",
  );
});
...
test("message part collection helpers update parts immutably without the DOM", () => {
  ...
  const replaced = replaceMessagePart(
    patched.messages,
    "m-1",
    "text-1",
    replacement,
  ...
  const missingPart = patchMessagePart(base, "m-1", "missing", {
    status: "complete",
  });
...
test("todo item patching is immutable, revision-aware, and updates lifecycle status", () => {
  ...
  const updated = patchTodoItem(
    ...
    "verify",
    {
      ...
      title: "Verify UI",
      status: "active",
    } as Parameters<typeof patchTodoItem>[2],
    2,
  );
  ...
  const invalidRevision = patchTodoItem(
    updated.part,
    "verify",
    { status: "done" },
    NaN,
...
test("todo expansion defaults only apply when a stable part id first appears", () => {
  ...
  assert.equal(
    shouldInitializeTodoExpansion(collapsed.id, collapsed.id),
    false,
  );
  ...
  assert.equal(
    shouldInitializeTodoExpansion(collapsed.id, dataUpdateSameId.id),
    false,
  );
});
