...
test('an element that already holds the render reuses its own HTML', () => {
  storeMarkdownCache('p1', 'hello', '<p>hello</p>', NO_OPTIONS);
  assert.deepEqual(lookupMarkdownCache('p1', 'hello', '<p>hello</p>', NO_OPTIONS), {
    reuse: 'previous',
  });
});
...
test('a different highlightJs instance misses', () => {
  ...
  storeMarkdownCache('p1', '```js\n1\n```', '<pre>a</pre>', { highlightJs: first });

  assert.deepEqual(lookupMarkdownCache('p1', '```js\n1\n```', '', { highlightJs: first }), {
    reuse: 'cached',
    html: '<pre>a</pre>',
  });
  assert.deepEqual(
...
test('gaining a highlighter misses', () => {
  ...
  assert.deepEqual(lookupMarkdownCache('p1', 'x', '<p>x</p>', { highlightJs: fakeHighlightJs() }), {
    reuse: 'none',
  });
});
...
test('a reordered protocol list is treated as a change', () => {
  ...
  assert.deepEqual(
    lookupMarkdownCache('p1', 'x', '<p>x</p>', { allowedLinkProtocols: ['b', 'a'] }),
    { reuse: 'none' },
  );
});
...
test('a fresh element re-renders rather than replay an unresolved placeholder', () => {
  ...
  assert.deepEqual(lookupMarkdownCache('p1', '```mermaid\ng\n```', '', NO_OPTIONS), {
    reuse: 'none',
  });
});
...
function fill(partId: string, chars: number): void {
  storeMarkdownCache(partId, 'c'.repeat(chars / 2), 'h'.repeat(chars / 2), NO_OPTIONS);
}
