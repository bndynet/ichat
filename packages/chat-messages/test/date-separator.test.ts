...
test("getDateSeparatorInfo returns days-ago for 2-7 days", () => {
  ...
  for (let days = 2; days <= 7; days++) {
    const ts = new Date(
      `2025-07-${String(25 - days).padStart(2, "0")}T12:00:00Z`,
    ).getTime();
    const info = getDateSeparatorInfo(ts, DATE_SEPARATOR_LABELS_EN, now);
...
test("getDateSeparatorInfo returns Chinese labels", () => {
  ...
  assert.equal(
    getDateSeparatorInfo(today, DATE_SEPARATOR_LABELS_ZH_CN, now).label,
    "今天",
  );

  const yesterday = new Date("2025-07-24T12:00:00Z").getTime();
  assert.equal(
    getDateSeparatorInfo(yesterday, DATE_SEPARATOR_LABELS_ZH_CN, now).label,
    "昨天",
  );

  const threeDaysAgo = new Date("2025-07-22T12:00:00Z").getTime();
  assert.equal(
    getDateSeparatorInfo(threeDaysAgo, DATE_SEPARATOR_LABELS_ZH_CN, now).label,
    "3天前",
  );

  const older = new Date("2025-07-10T12:00:00Z").getTime();
  assert.equal(
    getDateSeparatorInfo(older, DATE_SEPARATOR_LABELS_ZH_CN, now).label,
    "更久以前",
  );
});
