...
const todayItems = buildMessageRenderItems(todayMessages, DATE_SEPARATOR_LABELS_EN);
assert.deepEqual(
  todayItems.map((item) => item.key),
  ['m-1', 'm-2'],
);
...
const mixedItems = buildMessageRenderItems(
  [{ ...todayMessages[0], timestamp: daysAgo(1) }, todayMessages[1]],
...
// Default is 'auto' — virtual scrolling enables automatically when messages exceed 500.
assert.equal(DEFAULT_CONFIG.virtualScroll, 'auto');
