...
export function buildMessageRenderItems(
  ...
  for (const message of messages) {
    const { key, label } = getDateSeparatorInfo(message.timestamp ?? fallbackTimestamp, labels);
...
export function findMessageRenderIndex(
  ...
  return items.findIndex((item) => item.kind === 'msg' && item.message.id === messageId);
}

export function findPartRenderIndex(items: readonly MessageRenderItem[], partId: string): number {
  return items.findIndex(
    (item) => item.kind === 'msg' && (item.message.parts ?? []).some((part) => part.id === partId),
  );
}
