...
export function morphHtmlInto(el: HTMLElement, html: string): void {
  ...
  morphdom(el, temp, {
    ...
    onBeforeElUpdated(fromEl, toEl) {
      if (
        fromEl.tagName === "I-CHAT-MERMAID" &&
        toEl.tagName === "I-CHAT-MERMAID"
      ) {
        return true;
      }
      if (
        fromEl.tagName === "I-CHAT-CODE-TOGGLE" &&
        toEl.tagName === "I-CHAT-CODE-TOGGLE"
      ) {
        return true;
      }
      if (fromEl.tagName.includes("-") && fromEl.tagName === toEl.tagName) {
        ...
        if (fromAttributes.length === toAttributes.length) {
          ...
          for (let i = 0; i < toAttributes.length; i++) {
            if (
              fromEl.getAttribute(toAttributes[i].name) !==
              toAttributes[i].value
            ) {
              same = false;
              break;
            }
