...
- [ ] 🔴 **[P0] Add a Store × ownership × lifecycle behavior matrix** — Node-level component regressions now cover initial external history, pre-connect and runtime mode changes, synchronous/asynchronous controlled acceptance, rejection, external replacement, and controlled `ChatRunController` streaming. Complete the matrix with every imperative mutation and rendered browser lifecycle coverage over both ownership modes.
