...
function refreshMetrics() {
  ...
  activeMode.value = root.querySelector("lit-virtualizer")
    ? "Virtual"
    : "Regular";
}
...
async function applyScenario(reason = "Scenario updated") {
  ...
  await settleChat(chat, virtualScrollEffective());
...
function cycleVirtualScroll() {
  // Cycle: auto → true → false → auto
  if (virtualScroll.value === "auto") virtualScroll.value = true;
  else if (virtualScroll.value === true) virtualScroll.value = false;
  else virtualScroll.value = "auto";
  applyScenario(`Virtual scroll: ${virtualScroll.value}`);
}

function virtualScrollLabel() {
  if (virtualScroll.value === "auto") return "Auto";
  return virtualScroll.value ? "On" : "Off";
}

function virtualScrollEffective() {
  const vs = virtualScroll.value;
  if (vs === "auto") return messageCount.value > 500;
  return !!vs;
}

function currentMountedMessageIndex() {
  const mountedIndexes = Array.from(
    chatRef.value?.shadowRoot?.querySelectorAll(
  ...
  <div class="virtual-scroll-demo">
    <div class="demo-toolbar">
      ...
      <div class="toolbar-control">
        ...
        <el-button
          size="small"
          :type="virtualScroll === 'auto' ? 'primary' : 'default'"
          :disabled="loading || streaming"
          @click="cycleVirtualScroll"
        >
          {{ virtualScrollLabel() }}
        </el-button>
      </div>
